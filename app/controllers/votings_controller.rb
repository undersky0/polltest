class VotingsController < ApplicationController
  ...
  def index
    @votings = Voting.all
    @data = Voting.voted
    @votes = @data.group_by {|k| k[:parliamentary] }
  end
  ...
  def create
    ...
    respond_to do |format|
      if @voting.save
        format.html { redirect_to votings_path, notice: 'Voting was successfully created.' }
        format.json { render :index, status: :created, location: @voting }
      else
  ...
  private
    ...
    def voting_params
      params.require(:voting).permit(:postcode, :parliamentary, :party, :vote)
    end
end
